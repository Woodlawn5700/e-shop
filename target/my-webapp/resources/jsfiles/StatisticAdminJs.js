/**
 * Created by pavelpetrov on 23.10.16.
 */
var minTime;
var maxTime;
$(function () {
    ...
    $("#getProfitBtn").click(function(event) {
        getProfit(event);
    });
...
function getProfit (event) {
    ...
    $.ajax({
        ...
        url: "/admin/GetProfit",
        data: {
            firstDate : minTime.val(),
            lastDate : maxTime.val()
        },
        success: function (result) {
            if (!result.success) {
                alert(result.message);
            } else {
                $('#profitDiv').load('../../StatisticAdminPage #profitDiv', function () {
                });
            }
        },
...
function changeColor() {
    document.getElementById("statisticHeader").style.background = "#2e6da4";
}
...
document.write('<table id="fc" style="position:absolute;border-collapse:collapse;background:#FFFFFF;border:1px solid #2e6da4;display:none;-moz-user-select:none;-khtml-user-select:none;user-select:none;" cellpadding="2">');
...
document.write('<tr style="background:#2e6da4;font:12px Arial;color:#FFFFFF"><td align=center>M</td><td align=center>T</td><td align=center>W</td><td align=center>T</td><td align=center>F</td><td align=center>S</td><td align=center>S</td></tr>');
...
document.write('<tr><td colspan="7" align="center" style="cursor:pointer;font:13px Arial;background:#2e6da4" onclick="today()">Today: '+addnull(sccd,sccm+1,sccy)+'</td></tr>');
...
function cs_over(e) {
    evtTgt(EvtObj(e)).style.background='#2e6da4';
}
function cs_out(e) {
...
function f_cps(obj) {
    ...
    obj.style.color='#2e6da4';
    obj.style.textAlign='center';
    obj.style.textDecoration='none';
    obj.style.border='1px solid #2e6da4';//'1px solid #606060';
